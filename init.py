...
def sendToAll(data_d, conns):
    ...
    for conn in conns:
        ...
        while(st != 'ok'):
            try:
                t = len(ds)
                print("Connecting to {}:{}".format(conn,iport))
                sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                sock.connect((conn, iport))
                t = int(t)
                t = t.to_bytes(commsize, endian)
                sock.sendall(t)
                sock.sendall(ds)
                st = sock.recv(commsize).decode('utf-8')
    ...
    if servIp == ip:
        ...
        while(True):
            if(total_conn == tot_hosts - 1):
                break
            print("Tot conn {} tot hosts {}".format(total_conn, tot_hosts))
            print("Before accept")
            conn, client = sock.accept()
            print("After accept")
            conns.append(client[0])
        ...
        while(akc == ''):
            try:
                ...
                sock.sendall(message)
                akc = sock.recv(commsize)
                akc = akc.decode('utf-8')
                # print(rankToSock)
            except Exception as e:
                traceback.print_exc()
        ...
        sock.bind((ip, iport))
        sock.listen(5)
        print("Listening on {}:{}".format(ip,iport))
        conn, client = sock.accept()
        d = ''
        with conn:
            chunks = []
        ...
        print("Accepted on {}:{}".format(ip,iport))
        sock.close()
        # sock.sendall('ok'.encode('utf-8'))
        rankToSock = pickle.loads(d)
        sockToRank = dict((v, k) for k, v in rankToSock.items())
        ...
        for idx, key in enumerate(offsets):
            ...
            for i1, _ in enumerate(range(key[0]//c, (key[1]+1)//c)):
                ...
                if idx == myrank:
                    ...
                    #operate on row
                    print("Source col {} rank {} divsion by {}".format(col, rank, aa[i1,col]))
                ...
                #not my rank
                    ...
                    if(stat == 'help'):
                        ...
                        div = stat
                        print("dest col {} rank {} divsion by {}".format(col, rank, div[col]))
                        if(div[col] == 0):
                            print("Zero div expanded {}".format(div))
                        # print(aa[0])
                        for iy in range(aa.shape[0]):
                            aa[iy,:] -= (aa[iy,col]/div[col])*div
                    tagc += 1
        ...
        try:
            for idx, key in reversed(list(enumerate(offsets))):
                ...
                for i1, _ in reversed(list(enumerate(range(key[0]//c, (key[1]+1)//c)))):
                    col -= 1
                    print("col IS {} for rank {} and i1 {} and idx {}".format(col, myrank, i1, idx))
                    if col == 0:
                        if(idx == myrank):
                            if(aa[i1,col] != 0):
                                aa[i1,:] = aa[i1,:] / aa[i1,col]
